...
from utils import get_batch_index, get_inputs, get_inputs2
...
class RAM(object):
    ...
    def build_model(self):
        with tf.name_scope('inputs'):
            ...
            self.sentences = tf.placeholder(tf.float32, [None,self.max_sentence_len, 768])
            self.aspects = tf.placeholder(tf.float32, [None,self.max_aspect_len, 768])
            ...
            # inputs = tf.convert_to_tensor(self.sentences)
            inputs = tf.nn.dropout(self.sentences, keep_prob=self.dropout_keep_prob)
            # aspect_inputs = tf.nn.embedding_lookup(self.word2vec, self.aspects)
            # aspect_inputs = tf.convert_to_tensor(self.aspects)
            aspect_inputs = tf.reduce_mean(self.aspects, 1)
    ...
    def train(self, data):
        # sentences,segment_ids,word_indexer,aspect_indexer, aspects, sentence_lens, sentence_locs, labels = data
        sentence_vec,aspect_vec,sentence_lens, sentence_locs, labels = data
        cost, cnt = 0., 0

        for sample, num in self.get_batch_data(sentence_vec,aspect_vec, sentence_lens, sentence_locs, labels, self.batch_size, True, self.dropout):
    ...
    def test(self, data):
        ...
        for sample, num in self.get_batch_data(sentence_vec,aspect_vec, sentence_lens, sentence_locs, labels, int(len(sentence_vec) / 2) + 1, False, 1.0):
